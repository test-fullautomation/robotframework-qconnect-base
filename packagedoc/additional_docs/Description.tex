...
\textbf{transfer file}
\texttt{conn\_name={[}conn\_name{]}\ \ \ src={[}source{]}\ \ \ dest={[}destination{]}\ \ \ transfer\_type={[}transfer type{]}}
\end{quote}
...
\textbf{transfer\_type}: 'put' to send from local to remote, 'get' to bring it back.
\end{quote}
\end{quote}

\hypertarget{description-transfer-folder}{%
\subsection{\texorpdfstring{\textbf{transfer folder}}{transfer folder}}\label{description-transfer-folder}}

\begin{quote}
\textbf{Use for transfering folder from local to remote and vice versa. (Only available for SSHClient connection type)}

\textbf{Syntax}:

\begin{quote}

\textbf{transfer folder}
\texttt{conn\_name={[}conn\_name{]}\ \ \ src={[}source{]}\ \ \ dest={[}destination{]}\ \ \ transfer\_type={[}transfer type{]}}
\end{quote}
