...
\item \textbf{fetch\_block}:
  \begin{quote}
    If \rcode{True}, every response line will be put into a block untill a line match \textbf{eob\_pattern} pattern.
...
\begin{quote}
\textbf{A corresponding match object (list-like) if a match is found.}

The match object contains both the full string matched by the entire
pattern and the substrings matched by individual capturing groups
within the pattern.

For experimenting and verifying regular expressions interactively,
see \href{https://regex101.com/}{regex101}.
...
\item
  ...
  i.e.\ the entire string segment that matches the full search pattern.
\item
  \$\{result\}{[}1{]} will be \textbf{"1st"},
  i.e.\ the first \emph{captured group} defined in the pattern \rcode|([0-9]..)|.
\item
  \$\{result\}{[}2{]} will be \textbf{"command"},
  i.e.\ the second \emph{captured group} defined in the pattern \rcode|(command)|.
\end{itemize}

In general:
\begin{itemize}
\tightlist
\item \${result}[0] = the full match of the entire regular expression.
\item \${result}[n] (with $n \geq 1$) = the $n$-th capturing group within the pattern.
\end{itemize}
