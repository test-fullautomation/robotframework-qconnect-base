...
\item \textbf{search\_pattern}:
  \begin{quote}
    ...
    It will match:
    \begin{itemize}
      \item a single line by default (\textbf{fetch\_block} not used)
      \item multiple lines if \textbf{fetch\_block} is enabled
    \end{itemize}
...
\item \textbf{timeout}:
  \begin{quote}
    Timeout (in seconds) for each \textbf{match\_try} (if used) while waiting for the response to match the pattern.
...
\item \textbf{fetch\_block}:
  \begin{quote}
    If \rcode{True}, every response line will be put into a block until a line match \textbf{eob\_pattern} pattern.

    Default value: \rcode{False}.
  \end{quote}

\item \textbf{eob\_pattern}: Applicable only when \textbf{fetch\_block} is \rcode{True}.
  \begin{quote}
    Regular expression for matching the endline when using \textbf{fetch\_block}.

    Default value: \rcode{.*}.
  \end{quote}

\item \textbf{filter\_pattern}: Applicable only when \textbf{fetch\_block} is \rcode{True}.
  \begin{quote}
    Regular expression for filtering every line to put into the block of response when using \textbf{fetch\_block}.
...
\begin{robotcode}
  ${result} = verify  conn_name=SSH_Connection
  ...                 fetch_block=True
  ...                 search_pattern=.*Starting Regular background program processing daemon.*
