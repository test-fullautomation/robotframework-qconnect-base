...
QCB-TCPIP-GC-100
    ...
    THREAD    VERIFY-THREAD-1     False
        conn_manager.verify    conn_name=QCB-TCPIP-GC-100-Connection    search_pattern=DELAY20-GC-100-T1    match_try=30    send_cmd=DELAY20-GC-100-T1
        send_thread_notification    VERIFY_THREAD_1_DONE
    END

    THREAD    VERIFY-THREAD-2     False
        FOR    ${index}    IN RANGE    0    15
            conn_manager.verify    conn_name=QCB-TCPIP-GC-100-Connection    search_pattern=GC-100-T2-${index}    match_try=20    send_cmd=GC-100-T2-${index}
        END
