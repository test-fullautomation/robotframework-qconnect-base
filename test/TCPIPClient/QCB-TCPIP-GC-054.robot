...
QCB-TCPIP-GC-054
    [Documentation]    Fetch block (5)
    ...                Filter pattern defined, search pattern defined (with two capturing groups).
    ...                Expected result is a list containing two elements containing the captured content.
    ...                The captured content is limited to the lines filtered by the filter pattern.

    # supports HTML overview
    set_test_variable    ${connection_type}    TCPIPClient
    set_test_variable    ${test_category}      GOODCASE

    set_test_variable    ${connection_name}    QCB-TCPIP-GC-054-Connection
    ...
    log    TCPIP-GC-054 'verify' status: ${status}    console=yes
    log    TCPIP-GC-054 'verify' result: ${result}    console=yes

    should_be_equal     ${status}       PASS
    length_should_be    ${result}       2
    should_be_equal     ${result}[0]    transmission point 1 passed\r\ntransmission point 2 passed\r\ntransmission point 3 passed
    should_be_equal     ${result}[1]    0
